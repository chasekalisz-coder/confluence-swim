...
### Progression demo for non-Silver/non-Gold tiers

First section to actually swap its data layer based on tier instead of just decorating with badges. Pattern landed here will likely be reused for the other gated sections (Power Rankings, Championship Standards, Age-Up, Range) as those each get their demo wiring.

The conversation that led here: Chase asked about progression demo ideas. Race Pace gating worked by throttling generations against a 5-day window — Progression doesn't have a "generation" concept, it's just data. So the question was whose data to show. Three options on the table: (1) Chase's own progression, labeled as such; (2) limit the user's own kid's data somehow; (3) static example chart not tied to anyone. Landed on option 1 — Chase already has the data (251 entries imported earlier in the project), it's a real Olympic medalist's career, and it's compelling without being arbitrary.

Sub-decision on which events: settled on 3 of Chase's career events in LCM. Chose 200 Fly, 200 IM, 400 IM — Chase's actual career story (warm-up event, World Championship medal event, Olympic medal event) and all in LCM because that's the international standard, the right course for an Olympic-progression story. SCY would have been the NCAA/college version. All three events have 10 LCM entries each in the source data, spanning 2017–2024 — roughly 7-8 years of progression, plenty for a meaningful curve.

Sub-decision on the chart UX: the existing `ProgressionChart` only renders one line at a time with a dropdown to switch events. Two paths considered — (A) keep that UX and limit the dropdown to the 3 demo events, or (B) modify the chart to render all 3 lines simultaneously. Chase: "1 line is better anyway just like the current tool works right." Went with A. The existing chart UX is preserved; the data swap is the entire change.

Implementation: new fixture file `src/data/progression-demo.js` exports `PROGRESSION_DEMO_DATA` — 30 entries pulled from `api/data/ath_chase.json` and filtered down to the 3 LCM events. Same `{event, time, date}` shape that `ProgressionChart` already consumes from `athlete.progression`, so the data plugs in directly. New `hasProgressionAccess` derivation in FamilyAnalysis (`tier === 'silver' || tier === 'gold'`); conditional in the JSX swaps the `data` prop and replaces the section lede with a `<div className="demo-banner">` for non-Silver/non-Gold.
